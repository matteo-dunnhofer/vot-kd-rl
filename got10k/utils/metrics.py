...
def bc_in_gt_frac(rects1, rects2):
    r"""Center error.
    ...
    ins = 0.0
    for i in range(rects1.shape[0]):
        cx, cy = rects1[i,0] + ((rects1[i, 2] - 1) / 2), rects1[i,1] + ((rects1[i, 3] - 1) / 2),
        if rects2[i,0] <= cx and cx <= rects2[i,0] + rects2[i,2] and rects2[i,1] <= cy and cy <= rects2[i,1] + rects2[i,3]:
            ins += 1


    return ins / rects1.shape[0]
