...
from ..utils.metrics import rect_iou, center_error, bc_in_gt_frac
...
class ExperimentOTB(object):
    ...
    def run(self, tracker, visualize=False):
        ...
        for s, (img_files, anno) in enumerate(self.dataset):
            ...
            boxes, times = tracker.track(
                img_files, anno[0, :], visualize=visualize, result_dir=self.result_dir, seq_name=seq_name, anno=anno)
            assert len(boxes) == len(anno)
            
            # record results
            self._record(record_file, boxes, times)

    def report(self, tracker_names, color_styles=None):
        ...
        for name in tracker_names:
            ...
            for s, (_, anno) in enumerate(self.dataset):
                ...
                bc_in_gt[s] = self._bc_in_gt(boxes, anno)
            ...
            avg_bc_in_gt = np.sum(bc_in_gt) / np.count_nonzero(bc_in_gt)
        ...
        self.plot_curves(tracker_names, color_styles=color_styles)

        return performance

    def report_per_attribute(self, tracker_names, colors_styles=None):
        assert isinstance(tracker_names, (list, tuple))

        with open(self.dataset.attributes_json_path) as json_file:
            attributes_json = json.load(json_file)

        # assume tracker_names[0] is your tracker
        report_dir = os.path.join(self.report_dir, tracker_names[0])
        if not os.path.isdir(report_dir):
            os.makedirs(report_dir)
        report_file = os.path.join(report_dir, 'performance_per_attribute.json')
        ...
        for name in tracker_names:
            ...
            for attribute in attributes_json:
                print('Evaluating attribute {} for tracker {}'.format(attribute, name))
                seq_names = attributes_json[attribute]
                ...
                for s, (_, anno) in enumerate(self.dataset):
                    ...
                    if seq_name in seq_names:
                        ...
                        boxes = np.loadtxt(record_file, delimiter=',')
                        boxes[0] = anno[0]
                        #print(seq_name, len(boxes), len(anno))
                        assert len(boxes) == len(anno)

                        ious, center_errors = self._calc_metrics(boxes, anno)
                        succ_curve[si], prec_curve[si] = self._calc_curves(ious, center_errors)
                ...
                # store overall attribute performance
                performance[name].update({ attribute: {
                    'success_curve': succ_curve.tolist(),
                    'precision_curve': prec_curve.tolist(),
                    'success_score': succ_score,
                    'precision_score': prec_score,
        ...
        with open(report_file, 'w') as f:
            json.dump(performance, f, indent=4)
        # plot precision and success curves
        self.plot_curves_per_attribute(tracker_names, colors_styles=colors_styles)

        return performance

    """
    def report_decisions_pr(self, tracker_name, expert_tracker_names, colors_styles=None):
        assert isinstance(tracker_names, (list, tuple))
        ...
        if not os.path.isdir(report_dir):
            os.makedirs(report_dir)
        report_file = os.path.join(report_dir, 'performance_pr.json')
        ...
        for s, (_, anno) in enumerate(self.dataset):
            ...
            if seq_name in seq_names:
                ...
                expert_tracker_results = {}
                for name in expert_tracker_names:
                    record_file = os.path.join(
                        self.result_dir, name, '%s.txt' % seq_name)
                    boxes = np.loadtxt(record_file, delimiter=',')
                    boxes[0] = anno[0]
                    ...
                    expert_tracker_results[name] = boxes

                gt_fusion_anno = [anno[0]]
                gt_fusion_tracker_idxs = [-1]

                ious_t = []
                for t in range(1, len(anno)):
                    for name in tracker_names:
                        ious_t.append(self._calc_metric(expert_tracker_results[name][t], anno[t]))

                    best_tracker_idx = argmax(ious_t)
                    best_bb = tracker_names[tracker_names[best_tracker_idx]][t]
                    gt_fusion_anno.append(best_bb)
                    gt_fusion_tracker_idxs.append(best_tracker_idx)

                performance[tracker_name]['seq_wise'].update({seq_name: {
                    'precision': prec,
                    'recall': rec }})
    ...
    def show(self, tracker_names, seq_names=None, play_speed=1):
        ...
        for s, seq_name in enumerate(seq_names):
            ...
            for f, img_file in enumerate(img_files):
                ...
                for name in tracker_names:
                    print(f, name, rect_iou(np.array([anno[f]]), np.array([records[name][f]])))
                time.sleep(1)
    ...
    def _bc_in_gt(self, boxes, anno):
        # can be modified by children classes
        bc = bc_in_gt_frac(boxes, anno)
        return bc

    def plot_curves(self, tracker_names, color_styles=None):
        # assume tracker_names[0] is your tracker
        report_dir = os.path.join(self.report_dir, tracker_names[0])
        ...
        if color_styles is None:
            colors = {t: '' for t in tracker_names}
            markers = ['-', '--', '-.']
            markers = [c + m for m in markers for c in [''] * 10]
        else:
            colors = {t: color_styles[t][0] for t in tracker_names}
            markers = {t: color_styles[t][1] for t in tracker_names}
    ...
    def plot_curves_per_attribute(self, tracker_names, colors_styles=None):
        ...
        for attribute in attributes_json:
            ...
            succ_file = os.path.join(report_dir, attribute + '_success_plots.png')
            prec_file = os.path.join(report_dir, attribute + '_precision_plots.png')
            ...
            # markers
            markers = ["-"]#, "--", "-."]
            if colors_styles is None:
                colors = {t: '' for t in tracker_names}
                markers = {t: '-' for t in tracker_names}
            else:
                colors = {t: colors_styles[t][0] for t in tracker_names}
                markers = {t: colors_styles[t][1] for t in tracker_names}
            ...
            ax.set(xlabel='Overlap threshold',
                   ylabel='Success rate',
                   xlim=(0, 1), ylim=(0, 1),
                   title='Success plots of OPE - ' + attribute)
            ax.grid(True)
            #fig.tight_layout()
            #ax.axis('equal')
            ...
            ax.set(xlabel='Location error threshold',
                   ylabel='Precision',
                   xlim=(0, thr_ce.max()), ylim=(0, 1),
                   title='Precision plots of OPE - ' + attribute)
            ax.grid(True)
            #fig.tight_layout()
            #ax.axis('square')
            ax.set_aspect('auto')
            #self.make_square_axes(ax.gca())

            print('Saving precision plots to', prec_file)
            fig.savefig(prec_file, bbox_extra_artists=(legend,),
                        bbox_inches='tight',
                        dpi=300)

    def make_square_axes(self, ax):
    ...
    def plot_per_sequence(self, tracker_names):
        # assume tracker_names[0] is your tracker


        succ_file = os.path.join('per_sequence_success_plots.png')
        prec_file = os.path.join('per_sequence_precision_plots.png')
        key = 'overall'
        ...
        for name in tracker_names:
            ...
            succ_scores = []
            for s, _ in enumerate(self.dataset):
                seq_name = self.dataset.seq_names[s]
                succ_scores.append(performance[name]['seq_wise'][seq_name]['success_score'])

            tracker_scores.update({name: succ_scores})
        ...
        legends = []
        for i, name in enumerate(tracker_names):
            #print(n_seqs, len(tracker_scores[name]))
            line = ax.scatter(list(range(n_seqs)),
                            tracker_scores[name])#,
                            #markers[i % len(markers)])
            points.append(line)
            legends.append('%s' % (name))
        matplotlib.rcParams.update({'font.size': 7.4})
        legend = ax.legend(points, legends, loc='center left',
                           bbox_to_anchor=(1, 0.5))

        matplotlib.rcParams.update({'font.size': 9})
        ax.set(xlabel='Sequences',
               ylabel='Success score',
               xlim=(0, n_seqs), ylim=(0, 1),
               title='Per-sequence success plots of OPE')
        ax.grid(True)
        fig.tight_layout()
