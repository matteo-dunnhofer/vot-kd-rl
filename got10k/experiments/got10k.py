...
class ExperimentGOT10k(object):
        ...
        for s, (img_files, anno) in enumerate(self.dataset):
            ...
            for r in range(self.repetitions):
                ...
                boxes, times = tracker.track(
                    img_files, anno[0, :], visualize=visualize, result_dir=self.result_dir, seq_name=(seq_name + '/' + seq_name + '_001'), anno=anno)
    ...
    def plot_curves(self, report_files, tracker_namess):
        assert isinstance(report_files, list), \
            'Expected "report_files" to be a list, ' \
            'but got %s instead' % type(report_files)
        
        # assume tracker_names[0] is your tracker
        report_dir = os.path.join(self.report_dir, tracker_namess[0])
        if not os.path.exists(report_dir):
            os.makedirs(report_dir)
